normalized_to_dna <- function(dir_filtered,
  ...
  # Load in counts list.
  counts_list <- readRDS(paste0(dir_filtered, filtered_counts_name))
  region_pairs <- list(c("Ant_Lateral" = 1, "Post_Lateral" = 5), 
                       c("Ant_Medial" = 2, "Post_Medial" = 6),
                       c("Ant_Nasal" = 3, "Post_Nasal" = 7),
                       c("Ant_Oral" = 4, "Post_Oral" = 8))
  ...
  pathway_list <- get_reactome_pathways(file_name = "Mus_musculus_10to100_nonredundant0.9")
  ...
  for(i in 1:length(region_pairs)) {
    # Perform dna on particular pathway for a particular region pair.
    pair <- region_pairs[[i]]
    pair_name <- gsub("(.*_)", "", names(pair)[1])
    
    # Perform differential network analysis on each pathway.
    results_by_lp <-  dna(counts_list[pair], pathway_list, n_perm = 1000, 
    ...
    for(k in 1:length(results_by_lp)) {
      ...
      save_file <- paste0(dir_results, "lp", lp_set[k], "_", pair_name, ".rds")
      cat("Saving dna results to", save_file, "\n")
      saveRDS(results_list, save_file)
      
    } # End for k in lp_set
  } # End for i in region pairs
}