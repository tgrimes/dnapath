entrez_to_symbol <- function(df, species,
  ...
  if(species == 1) {
    ...
    # Mouse
    load_file <- paste0(dir_annotation, "entrez_to_mgi.rds")
    if(file.exists(load_file)) {
      cat("\t- loading gene info from", load_file, "\n")
      gene_info <- readRDS(load_file)
    } else {
      ...
      gene_info <- getBM(attributes = c("entrezgene", "mgi_symbol"), 
                         mart = mart_mouse)
      gene_info %>%
        group_by(entrezgene) %>%
        summarise(mgi_symbol = first(mgi_symbol)) ->
        gene_info
