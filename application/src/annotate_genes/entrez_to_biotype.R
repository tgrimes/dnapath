entrez_to_biotype <- function(df, species,
  ...
  if(species == 1) {
    # Human
    load_file <- paste0(dir_annotation, "entrez_to_biotype_hgnc.rds")
    if(file.exists(load_file)) {
      cat("\t- loading gene info from", load_file, "\n")
      gene_info <- readRDS(load_file)
    } else {
      if(!("mart_human" %in% ls())) init_mart_human()
      gene_info <- getBM(attributes = c("entrezgene", "gene_biotype"), 
                         mart = mart_human)
      gene_info %>%
        filter(entrezgene != "", gene_biotype != "") ->
        gene_info
    ...
    # Mouse
    load_file <- paste0(dir_annotation, "entrez_to_biotype_mgi.rds")
    if(file.exists(load_file)) {
      load_file <- "data/annotations/entrez_to_biotype_mgi.rds"
      cat("\t- loading gene info from", load_file, "\n")
      gene_info <- readRDS(load_file)
    } else {
