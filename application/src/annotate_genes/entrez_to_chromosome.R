entrez_to_chromosome <- function(df, species,
  ...
  if(species == 1) {
    # Human
    load_file <- paste0(dir_annotation, "entrez_to_chromosome_hgnc.rds")
    if(file.exists(load_file)) {
      cat("\t- loading gene info from", load_file, "\n")
      gene_info <- readRDS(load_file)
    } else {
      if(!("mart_human" %in% ls())) init_mart_human()
      gene_info <- getBM(attributes = c("entrezgene", "chromosome_name"), 
                         mart = mart_human)
      gene_info %>%
        filter(entrezgene != "", chromosome_name != "") ->
        gene_info
    ...
    # Mouse
    load_file <- paste0(dir_annotation, "entrez_to_chromosome_mgi.rds")
    if(file.exists(load_file)) {
      cat("\t- loading gene info from", load_file, "\n")
      gene_info <- readRDS(load_file)
    } else {
